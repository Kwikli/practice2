<template>
    <div class="weather_content">
        Напишіть місто за замовчуванням
        <input type="text" v-model="defaultCity" @keyup="setDefaultCity()"> Місто: - {{defaultCity}}
    </div>
    ...
    <div class="weather_content">
        <div class="row50">
            <div class="col50">
                Додавання міста
                <br><input type="text" v-model="searchCity" placeholder="Місто...">
                <br><button @click="searchWeatherCity()">Додати місто</button>
            </div>
            <div class="col50">
                Вибір міста
                <br><select v-model="city">
                    <option value="" selected hidden>Оберіть місто</option>
                    <option v-for="(item,index) in citys" v-bind:key="index" :value="item">{{item}}</option>
                </select>
                <router-link :to="'/weather-forecast/'+city">
                    <br><button>Дізнатися погоду</button>
                </router-link>
            </div>
        </div>
    </div>
...
function showPosition(position) {
    ...
    axios.get("https://api.openweathermap.org/data/2.5/weather?lat="+position.coords.latitude+"&lon="+position.coords.longitude+"&appid=7914d5a440960cfd5df3bd0388a7ad0f").then(data => {
        console.log(data);
        window.localStorage.setItem('yourcity',data.data.name);
    });
}
...
export default {
    ...
    methods: {
        searchWeatherCity(){
            axios.get("https://api.openweathermap.org/data/2.5/weather?q="+this.searchCity+"&appid=7914d5a440960cfd5df3bd0388a7ad0f").then(data => {
                if(data){
                    console.log('data');
        ...
        getData(){
            axios.get("https://api.openweathermap.org/data/2.5/weather?q="+window.localStorage.getItem('defaultcity')+"&appid=7914d5a440960cfd5df3bd0388a7ad0f").then(data => {
